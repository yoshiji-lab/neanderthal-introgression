...
fenland <- fread("/scratch/richards/chen-yang.su/01.pQTL/01.fenland/03.cis_pQTL/all_strict_v2g_cis_res.tsv")
fenland[fenland$gene == "OAS1",]$protein  # OAS1.10361_25 
fenland[fenland$gene == "BMP1",]$protein  # BMP1.3348_49 
fenland[fenland$gene == "BMP1",]  
pqtl_chr <- fenland[fenland$gene == "BMP1",]$chromosome
pqtl_pos <- fenland[fenland$gene == "BMP1",]$base_pair_location
pqtl_rsid <- fenland[fenland$gene == "BMP1",]$rs_id

# Fenland Protein GWAS (hg19)
bmp1 <- fread("/scratch/richards/restricted/somalogic_mrc-epid.cam.ac.uk/res_invn_X3348_49_Fenland_MA_auto_chrX_filtered_1pc.txt.gz")
...
write_tsv(bmp1_region, "/scratch/richards/chen-yang.su/24.atlas/03.haplotype/bmp1_regionGRCh37_Fenland.tsv")
...
region <- data.frame(
  chr = paste0(pqtl_chr),
  start = pqtl_pos - 10000000, # GRCh37
  end = pqtl_pos + 10000000    # GRCh37
)
write.table(region, file = "/scratch/richards/chen-yang.su/24.atlas/03.haplotype/BMP1_region_hg37_PLINK.txt", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
...
cmd <- paste0(
  ...
  "--ld-window-kb 200 ",
  "--ld-window-r2 0 ",
  "--out /scratch/richards/chen-yang.su/24.atlas/03.haplotype/ld_output"
)
system(cmd)   # runs it

# 🔹 Step 3: Filter SNPs with r² > 0.8
ld <- read.table("/scratch/richards/chen-yang.su/24.atlas/03.haplotype/ld_output.ld", header=TRUE)
...
print(haplo_length)
write.table(haplo_df, file = "/scratch/richards/chen-yang.su/24.atlas/03.haplotype/BMP1_region_hg38.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
...
system(paste0("~/projects/richards/share/bin/bedtools map -a /scratch/richards/chen-yang.su/24.atlas/03.haplotype/BMP1_region_hg38.bed -b /scratch/richards/chen-yang.su/data/28.recombRate/bedGraph/recomb1000GAvg.bedGraph -c 4 -o mean"))
...
# Test yourself ----
toy2 <- data.frame(
  region_id = c("BMP1 Fenland haplotype"),
  length_bp = c(haplo_length),
  # recomb_cM_Mb = c(6.14818)
  recomb_cM_Mb = c(4.298891667)
  # recomb_cM_Mb = c(3.804968)
)
